...
node t[MaxN << 5];
int n, tot, a[MaxN], v[MaxN], dep[MaxN];
...
int main()
{
    ...
    while (T--)
    {
        ...
        n = read(), tot = 0;
        for (int i = 1; i <= n; i++)
            a[i] = read(), v[i] = dep[i] = 0;
        st.build(1, 1, n), q.push(1);
        now = 1, v[1] = 1, dep[1] = 0;
        while (!q.empty())
        {
            while (v[a[now]] > 0)
            ...
            if (now > n) break;
            int tmp = a[now];
            q.push(tmp), st.modify(1, , tmp), v[tmp] = 1, dep[tmp] = dep[q.front()] + 1;
            while (t.query_sub(t.root, tmp) != inf)
            {
                tmp = t.query_sub(t.root, tmp), q.push(tmp), v[tmp] = 1;
                t.del(t.root, tmp), dep[tmp] = dep[q.front()] + 1;
                printf("%d %d\n", now, tmp);
            }
            q.pop();
        }
        int ans = 0;
        for (int i = 1; i <= n; i++)
            ans = std::max(ans, dep[a[i]]), printf("%d ", dep[a[i]]);
        printf("%d\n", ans);
    }
    return 0;
}