...
int n, m, k, lim, c[MaxN];
int w[MaxN], rev[MaxN], _inv[MaxN];
...
signed main()
{
    n = read(), m = read(), __init();
    a.resize(100001), init(300000);
    for (int i = 1; i <= n; i++)
        scanf("%d", &c[i]), a[c[i]] = 1;
    a.mul(-4), a[0] = (a[0] + 1) % mod, a.sqrt();
    a[0] = (a[0] + 1) % mod, a.inv(), a.mul(2);
    for (int i = 1; i <= m; i++)
        printf("%d\n", a[i]);
    return 0;
}